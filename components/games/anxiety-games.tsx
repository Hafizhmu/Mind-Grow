  ...
  const handleGameStart = async (gameId: string) => {
    ...
    if (onGamePlayed) {
      try {
        await onGamePlayed(
          gameId,
          games.find((game) => game.id === gameId)?.description || ""
        );
      } catch (error) {
  ...
  return (
    <>
      <Card className="border-primary/10">
        <CardHeader>
          <CardTitle className="text-xl font-semibdol flex items-center gap-2">
            <Gamepad2 className="w-5 h-5 text-primary" /> Anxiety Relief
            Activities
          </CardTitle>
          <CardDescription>
            Interactive exercises to help reduce anxiety and stress
          </CardDescription>
        </CardHeader>
        <CardContent>
